...
class TracesLoginService:
    ...
    async def resend_captcha(self, session_id: str):
        try:
            session_manager = await get_session_manager()
            session = await session_manager.get_session(session_id)
            ...
            # Fetch new captcha via API
            captcha_id, captcha_base64, _ = await self.automation.get_captcha_api()

            # Update session with new captcha details
            ...
            return {
                ...
                "captcha_id": captcha_id,
                "captcha_base64": captcha_base64,
                "message": "New captcha generated via API",
            }
            ...
            logger.error(
                f"Error resending captcha for session {session_id}: {str(e)}",
                exc_info=True,
            )
            raise
