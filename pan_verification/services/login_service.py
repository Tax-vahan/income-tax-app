...
from pan_verification.core import get_session_manager
from pan_verification.utils.logger import get_logger
from pan_verification.utils.errors import SessionExpiredError, PortalTimeoutError, InvalidCredentialsError
...
class TracesLoginService:
    ...
    async def login(self, session_id: str, tan: str, password: str, captcha: str) -> dict:
        """
        ...
        if session is None:
            ...
            raise SessionExpiredError("Session not found or expired. Please request a new captcha.")
        ...
        try:
            ...
            if not captcha_id:
                raise SessionExpiredError("captcha_id not found in session. Please request a new captcha.")

            logger.info(f"User provided captcha: {captcha}")

            # Validate captcha is provided
            if not captcha or not captcha.strip():
                raise InvalidCredentialsError("Captcha text is required. Please enter the code from the image.")
    ...
    async def auto_login(self, tan: str, password: str) -> str:
        ...
        for attempt in range(max_attempts):
            try:
                logger.info(f"Auto-login attempt {attempt + 1}/{max_attempts} for TAN: {tan}")
    ...
    async def login_with_fallback(self, session_id: str, tan: str, password: str, captcha: str) -> dict:
        """
        ...
        try:
            ...
            try:
                session_data = await self.automation.submit_login_api(tan, password, captcha, captcha_id)
                ...
                if "403" in error_msg or "forbidden" in error_msg:
                    logger.warning("Password login failed with 403 - falling back to OTP...")
                else:
                    raise

            # Fall back to OTP login if password fails
            logger.info("Attempting OTP-based login as fallback...")
            logger.warning("OTP login requires user to manually provide OTP code via SMS/Email")
            logger.warning("This is a fallback method when API password authentication is restricted")
