...
from ..utils.config import TAXVAHAN_API_BASE, TAXVAHAN_API_KEY
...
def _fetch_page(
    ...
    headers = {"Authorization": f"Bearer {TAXVAHAN_API_KEY}"}
...
def fetch_manual_challans(
    ...
    page = 1
    while True:
        data  = _fetch_page(deductor_id, financial_year, quarter, category_id, page, page_size, timeout)
